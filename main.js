...
if (coefficientsString) {
  coefficients = coefficientsString.split(separatorSymbol);
}


getSignal.onclick = function() {
  let signal = coefficients[currentSignalIndex].toString();
  ...
  printSignal.innerHTML = `${signal}x`;
  
  currentSignalIndex++;
  if (currentSignalIndex >= coefficients.length){ 
...
function goTimer(time) {
  ...
  const timer = setInterval(() => {
    if (time >= 1) {
      ...
      stopProgress.style["animation"] = `animateProgress ${timerDuration}s linear infinite`;
